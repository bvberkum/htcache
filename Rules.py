	...
	def parse(klass, fpath=None):
		...
		if os.path.isfile(fpath):
			...
			try:
				...
				print ("Error parsing %s lines", klass.__name__)
				raise
			...
			for line in lines:
				try:
					...
					print ("Error parsing %s line: %r", klass.__name__, line)
					raise

		else:
			print ("No such file: %s", fpath)
		...
		if klass.rules:
			for pattern, regex, repl in klass.rules:
				...
				if m:
					...
					print ("Joined URL matching rule %r", pattern)
		return pathref
	...
	def run(klass):
		...
		for root, dirs, files in os.walk(Params.ROOT):
			...
			for f in files:
				...
				if fpath2 != fpath3:
					print ('FIXME: Renaming: %s --> %s', fpath2, fpath)
					continue # XXX
					print fpath2, fpath3
...
def load():
	...
	for x in Drop, NoCache, Join, Rewrite:
		x.main_file = getattr(Params, x.__name__.upper() + "_FILE")
		x.parse()
		print 'Loaded', x.__name__, len(x.rules)
