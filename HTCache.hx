...
class HTCache {
    ...
    static function onLoad(evt) {
        ...
        var 
        loc = js.Lib.window.location.href.toString();
        xhr.onreadystatechange = function() {
            if (xhr.readyState == 4) {
                var data = js.Lib.eval(xhr.responseText);
                untyped window.console.log("Response: "+data);
            }
            else {
                untyped window.console.log("Readystate "+xhr.readyState);
            }
        };
        xhr.setRequestHeader("Referer", loc);
        xhr.open("get", "http://"+ proxy_hostinfo +"/page-info", true);
        //xhr.send(loc);
        untyped window.console.log(xhr);
